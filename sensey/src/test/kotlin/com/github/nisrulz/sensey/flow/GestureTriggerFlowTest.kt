...
class GestureTriggerFlowTest {
    ...
    @Test
    fun observeReturnsEmptyFlowByDefault() = runBlocking {
    ...
    @Test
    fun callbackFlowBridgesDispatcherEmissions() = runBlocking {
        val events = mutableListOf<Int>()
        val flow = kotlinx.coroutines.flow.callbackFlow<Int> {
    ...
    @Test
    fun overriddenObserveReturnsCustomFlow() = runBlocking {
    ...
    @Test
    fun overriddenObserveCanBeEmpty() = runBlocking {
    ...
    @Test
    fun callbackFlowCleansUpOnCancellation() = runBlocking {
        var cleanedUp = false
        val flow = kotlinx.coroutines.flow.callbackFlow<Int> {
            val dispatcher: (Int) -> Unit = { trySend(it) }
            dispatcher(42)
    ...
    private class TestTrigger : GestureTrigger<Unit> {
        override fun evaluate(values: FloatArray, timestamp: Long): Unit? = Unit
    }

    private class CustomFlowTrigger : GestureTrigger<String> {
        override fun evaluate(values: FloatArray, timestamp: Long): String? = "event"
        override fun observe() = flowOf("a", "b")
    }

    private class EmptyObserveTrigger : GestureTrigger<String> {
        override fun evaluate(values: FloatArray, timestamp: Long): String? = null
